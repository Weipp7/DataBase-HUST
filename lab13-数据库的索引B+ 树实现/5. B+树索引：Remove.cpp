...
namespace bustub
{
    INDEX_TEMPLATE_ARGUMENTS BPLUSTREE_TYPE::BPlusTree(
        std::string name,
        BufferPoolManager *buffer_pool_manager,
        const KeyComparator &comparator,
        int leaf_max_size,
        int internal_max_size) : index_name_(std::move(name)),
     ...
     */
    INDEX_TEMPLATE_ARGUMENTS void BPLUSTREE_TYPE::Remove(const KeyType &key)
    {
    ...
    bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node)
    {
        ...
        / / result 为true代表此时node为父页的首结点 bool result = FindSibling(node, sibling);
        ...
        if (
            node->GetSize() + sibling->GetSize() <= node->GetMaxSize())
        {
            / / 进行融合 int removedIndex;
            ...
            {
                / / 当前结点合并到前驱结点上 removedIndex = parentPage->ValueIndex(node->GetPageId());
        ...
        / / unpin node,
            node2 buffer_pool_manager_->UnpinPage(
    ...
    bool BPLUSTREE_TYPE::FindSibling(N *node, N *&sibling)
    {
        ...
        page = buffer_pool_manager_->FetchPage(
            parent->ValueAt(siblingIndex));
     ...
     */
    INDEX_TEMPLATE_ARGUMENTS void BPLUSTREE_TYPE::UpdateRootPageId(int insert_record)
    {
        ...
        if (insert_record != 0)
        {
            / / create a new record<index_name + root_page_id> in header_page header_page->InsertRecord(index_name_, root_page_id_);
        }
        else
        {
            / /
