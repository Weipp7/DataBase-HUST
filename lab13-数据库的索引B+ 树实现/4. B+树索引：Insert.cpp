 ...
 */
INDEX_TEMPLATE_ARGUMENTS bool BPLUSTREE_TYPE:: GetValue(
    const KeyType & key,
...
if (
    leafPage -> Lookup(key, & v, comparator_)
){result -> push_back(v);
 ...
 *  3.不断向下搜索，直至目标叶子结点
 */
INDEX_TEMPLATE_ARGUMENTS Page * BPLUSTREE_TYPE:: FindLeafPage(
    const KeyType & key,
    bool leftMost
){if (IsEmpty())
return nullptr;
...
if (pointer = = nullptr){printf(
    "FindLeafPage get root false"
);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS bool BPLUSTREE_TYPE::
Insert
(
        const KeyType & key,
...
if (IsEmpty()) { //printf("new tree start\n");
 ...
 */
INDEX_TEMPLATE_ARGUMENTS void BPLUSTREE_TYPE:: StartNewTree(
    const KeyType & key,
    const ValueType & value
){page_id_t pageId;
...
//更新当前B+树的根页ID root_page_id_ = pageId;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS void BPLUSTREE_TYPE:: UpdateRootPageId(int insert_record){HeaderPage * header_page = static_cast < HeaderPage * > (
    buffer_pool_manager_ -> FetchPage(HEADER_PAGE_ID)
);

if (insert_record != 0){/ / create a new record < index_name + root_page_id > in header_page header_page -> InsertRecord(index_name_, root_page_id_);

}
else{/ /
update
    root_page_id in header_page header_page -> UpdateRecord(index_name_, root_page_id_);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS bool BPLUSTREE_TYPE:: InsertIntoLeaf(
    const KeyType & key,
    const ValueType & value
...
if (
    page -> Lookup(key, & v, comparator_)
){/ / printf(
    "insert key already existed \n"
);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS template < typename N > N * BPLUSTREE_TYPE:: Split(N * node){page_id_t newPageId;

Page *newPage = buffer_pool_manager_->NewPage(&newPageId);

N *newNode = reinterpret_cast<N *>(newPage->GetData());

/ / 不同类型的结点均存在初始化以及分裂 if(node -> IsLeafPage()) newNode -> Init(
    newPageId,
 ...
 */
INDEX_TEMPLATE_ARGUMENTS void BPLUSTREE_TYPE:: InsertIntoParent(
...
/ / fetch page
and new page need to unpin page buffer_pool_manager_ -> UnpinPage(new_node -> GetPageId(), true);
...
else{Page * page = buffer_pool_manager_ -> FetchPage(parentId);
...
if (
    parentPage -> GetSize() > parentPage -> GetMaxSize()
...
}/ / unpin函数内部打开的父结点以及传入的新结点 buffer_pool_manager_ -> UnpinPage(parentPage -> GetPageId(), true);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS INDEXITERATOR_TYPE BPLUSTREE_TYPE:: Begin(){KeyType _;
 ...
 */
INDEX_TEMPLATE_ARGUMENTS INDEXITERATOR_TYPE BPLUSTREE_TYPE:: Begin(const KeyType & key){Page * page = FindLeafPage(key, false);
 ...
 *  返回IsEnd状态的B+树记录迭代器
 */
INDEX_TEMPLATE_ARGUMENTS INDEXITERATOR_TYPE BPLUSTREE_TYPE::
End(){
return
    INDEXITERATOR_TYPE(
        nullptr,
...
}template class BPlusTree < GenericKey < 4 >,
RID,
GenericComparator < 4 >>;
